...
16. **Validate**: invoke `common-sdd-verify-spec.workflow.md` after final evidence is ready; record `status: verified` in the stable spec path only after its explicit final approval. Do not move the folder or create lifecycle artifacts outside the spec.
