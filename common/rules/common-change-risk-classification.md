...
- `bash tools/validate-agent-catalog.sh` when rules, workflows, skills, or catalog documentation change;
- `git diff --check`;
- shell/YAML/JSON syntax checks when those files change.

No product acceptance, unit, HTTP, E2E, mutation, or coverage gate is required for the L0 change itself unless executable behavior changes. If the change belongs to a spec entering `verified`, the applicable security, clean-up, and coverage gates still apply; documentation-only validation records `coverage_scope: none`.
...
E2E, mutation, security, and full HTTP integration are conditional on the touched boundary. The common SDD approval sequence still applies to behavior changes; risk selects additional evidence, not permission to edit production code before the corresponding inside-out RED and scoped Gate 3.
...
- SDD artifacts required by the risk and the Gate 1–3 implementation sequence;
...
- independent Reviewer handoff before final validation;
...
| Final validation review | no for non-SDD catalog work | required for SDD specs | required | required |
...
| Critical E2E | no | no | no unless escalated to L3 | required |
| Mutation testing | no | optional | non-trivial logic | required |
| Security review | no for non-SDD catalog work | scope review; deep evidence when boundary changes | scope review; deep evidence when boundary changes | required with trust-boundary evidence |
| Project-wide coverage | no for catalog-only work | ≥90% before `verified` when production code is in scope | ≥90% before `verified` | ≥90% before `verified` |
...
- This risk matrix selects additional evidence such as mutation and critical E2E. It does not override spec validation: every spec entering `verified` runs `common-sdd-coverage-gate.workflow.md`; when production code is in scope, the project-wide result must be `>= 90%` with no affected-scope regression. Documentation-only specs record `coverage_scope: none` and prove that no production files changed.
