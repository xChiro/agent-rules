...
3. Frame the acceptance behavior with ATDD.
4. Use TDD: write or update the failing unit test first.
5. Create only the Application request/response records and Application ports needed now.
...
8. Add HTTP integration coverage when the public path touches adapters or local infrastructure.
...
Define these port interfaces in the Application layer, near the use case or application module that consumes them. Infrastructure adapters implement Application ports; Domain must not define infrastructure, persistence, messaging, clock, session, ID-generation, or external API ports.
