...
- `integration-tests`: `go test -tags=integration` for `tests/integration/http/...` and `tests/integration/infrastructure/...` against local resources.
...
7. Run the boundary suite through HTTP or the configured message event source with test-safe environment values and dummy credentials.
...
- Require both `unit-tests` and `integration-tests` before packaging/deploy.
...
When an existing pipeline has separate `test-integration` or `test-e2e` jobs, move their public boundary scenarios and local resource setup into `integration-tests`, placing adapter/resource checks under `integration/infrastructure`. Preserve service-specific commands, artifacts, branch conditions, environment variables, and deployment dependencies in the generated repository workflow and `verification.md`.
